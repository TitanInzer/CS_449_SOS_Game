# -*- coding: utf-8 -*-
"""
Created on Sat Dec  9 20:27:39 2023
...
def select_board_size():
    ...
    radio_ai_player1 = Radiobox(screen, WIDTH // 2 - 180, HEIGHT // 2 + 150, 9, caption='AI as Player 1')
    radio_ai_player2 = Radiobox(screen, WIDTH // 2 + 60, HEIGHT // 2 + 150, 10, caption='AI as Player 2')
    radio_ai_both = Radiobox(screen, WIDTH // 2 - 50, HEIGHT // 2 + 150, 11, caption='AI for Both')
    ai_player_modes = [radio_ai_player1, radio_ai_player2, radio_ai_both]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for option in ai_player_modes:
                    ...
                    if option.checked:
                        IS_AI_GAME = True
                        if option.caption == 'AI as Player 1':
                            AI_PLAYER_NUMBER = 1
                        elif option.caption == 'AI as Player 2':
                            AI_PLAYER_NUMBER = 2
                        else:
                            AI_PLAYER_NUMBER = 0  # Both players are AI
...
def perform_ai_move(game):
    if not IS_AI_GAME:
        return  # Skip AI move if it's not an AI game

    if AI_PLAYER_NUMBER == 1 and game.current_player == 'Player 1':
        ...
        if row is not None and col is not None:
            ...
            if game.place_symbol(row, col):
                game.switch_player()
                pygame.time.delay(1000)  # Introduce a 1-second delay before AI move

    elif AI_PLAYER_NUMBER == 2 and game.current_player == 'Player 2':
        row, col, ai_symbol = ai_move(game.board)
        if row is not None and col is not None:
            ai_player = 'Player 2'
            game.set_choice(ai_player, ai_symbol)
    ...
    elif AI_PLAYER_NUMBER == 0:
        # Both players are AI, so alternate between Player 1 and Player 2 AI moves
...
def save_game_state(game, filename):
    with open(filename, 'w') as file:
        ...
        file.write(f"Selected Game Mode: {SELECTED_MODE}\n")

        # Save the board state
        for row in game.board:
            file.write(" ".join(row) + "\n")

        # Save the current player and their choices
        file.write(f"Current Player: {game.current_player}\n")
        for player, choice in game.choices.items():
            file.write(f"{player}'s Choice: {choice}\n")

        # Save the scores
        for player, score in game.score.items():
            file.write(f"{player}'s Score: {score}\n")

        # Save the winner (if any)
        if game.winner:
...
def main():
    ...
    while True:
        ...
        if (IS_AI_GAME and AI_PLAYER_NUMBER == 0) or (AI_PLAYER_NUMBER == 1 and game.current_player == 'Player 1'):
        ...
        if game.is_draw() or game.winner:
            ...
            save_game_state(game, "game_state.txt")
